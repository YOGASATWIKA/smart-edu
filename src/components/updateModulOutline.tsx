...
export default function EditableOutlineDisplay({ modulId, onSave }: EditableOutlineDisplayProps) {
    const [modul, setModul] = useState<Modul | null>(null);
    const [editableOutline, setEditableOutline] = useState<Outline | null>(null);
    const [isLoading, setIsLoading] = useState<boolean>(true);
    const [isSaving, setIsSaving] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        const fetchModulDetails = async () => {
            ...
            try {
                const data = await getModulById(modulId);
                setModul(data);
                // Deep copy untuk state yang bisa diedit
                setEditableOutline(JSON.parse(JSON.stringify(data.outline)));
            } catch (err: any) {
                setError(err.message || 'Gagal memuat detail outline.');
            } finally {
    ...
    if (isLoading) return <div className="flex items-center justify-center min-h-[70vh]"><p>Memuat outline untuk diedit...</p></div>;
    if (error) return <div className="flex items-center justify-center min-h-[70vh] text-red-500">Error: {error}</div>;
    if (!modul || !editableOutline) return <div className="flex items-center justify-center min-h-[70vh]"><p>Data outline tidak ditemukan.</p></div>;
